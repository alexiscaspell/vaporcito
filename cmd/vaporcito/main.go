...
import (
	...
	"github.com/alexiscaspell/vaporcito/cmd/vaporcito/cli"
	"github.com/alexiscaspell/vaporcito/cmd/vaporcito/cmdutil"
	"github.com/alexiscaspell/vaporcito/cmd/vaporcito/decrypt"
	"github.com/alexiscaspell/vaporcito/cmd/vaporcito/generate"
	...
	"github.com/alexiscaspell/vaporcito/lib/vaporcito"
...
values "default" and "-" may be used. The former logs to DATADIR/vaporcito.log
...
func (options serveOptions) Run() error {
	...
	if err := vaporcito.EnsureDir(locations.GetBaseDir(locations.ConfigBaseDir), 0o700); err != nil {
	...
	if options.Upgrade {
		...
		if err == nil {
			...
			ldb, err = vaporcito.OpenDBBackend(locations.Get(locations.Database), config.TuningAuto)
			if err != nil {
				err = upgradeViaRest()
			} else {
				_ = ldb.Close()
...
func syncthingMain(options serveOptions) {
	...
	cert, err := vaporcito.LoadOrGenerateCertificate(
		locations.Get(locations.CertFile),
		locations.Get(locations.KeyFile),
	)
	if err != nil {
	...
	cfgWrapper, err := vaporcito.LoadConfigAtStartup(locations.Get(locations.ConfigFile), cert, evLogger, options.AllowNewerConfig, options.NoDefaultFolder, options.SkipPortProbing)
	if err != nil {
	...
	ldb, err := vaporcito.OpenDBBackend(dbFile, cfgWrapper.Options().DatabaseTuning)
	if err != nil {
	...
	app, err := vaporcito.New(cfgWrapper, ldb, evLogger, cert, appOpts)
	if err != nil {
...
func setupSignalHandling(app *vaporcito.App) {
...
func autoUpgrade(cfg config.Wrapper, app *vaporcito.App, evLogger events.Logger) {
	...
	for {
		...
		case event := <-sub.C():
			...
			if !ok || data["clientName"] != "vaporcito" || upgrade.CompareVersions(data["clientVersion"], build.Version) != upgrade.Newer {
				continue
			}
