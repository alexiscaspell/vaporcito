...
func Generate(l logger.Logger, confDir, guiUser, guiPassword string, noDefaultFolder, skipPortProbing bool) error {
	...
	if err := vaporcito.EnsureDir(dir, 0o700); err != nil {
		return err
	}
	...
	if err == nil {
		...
		cert, err = vaporcito.GenerateCertificate(certFile, keyFile)
		if err != nil {
	...
	if fs.IsNotExist(err) {
		if cfg, err = vaporcito.DefaultConfig(cfgFile, myID, events.NoopLogger, noDefaultFolder, skipPortProbing); err != nil {
