...
angular.module('vaporcito.core')
    ...
    .controller('VaporcitoController', function ($scope, $http, $location, LocaleService, Events, $filter, $q, $compile, $timeout, $rootScope, $translate) {
        'use strict';
        ...
        $scope.folderStatus = function (folderCfg) {
            ...
            // after restart vaporcito process state may be empty
        ...
        $scope.saveConfig = function () {
            ...
            // bug (see https://github.com/alexiscaspell/vaporcito/pull/9078).
        ...
        async function getGameSaveDataLocationFromTable(table, os) {
            ...
            var preferred = null;
            var windowsFallback = null;
            var steamFallback = null;
            var anyFallback = null;

            for (var i = 0; i < pathList.length; i++) {
                var system = getValueOrDefault(pathList[i], "System", "");
                var location = getValueOrDefault(pathList[i], "Location", null);
                if (!location) {
                    continue;
                }
                if (!anyFallback) {
                    anyFallback = pathList[i];
                }
                if (systemsMatch(system, "Windows") && !windowsFallback) {
                    windowsFallback = pathList[i];
                }
                if (systemsMatch(system, "Steam") && !steamFallback) {
                    steamFallback = pathList[i];
                }
                if (systemsMatch(system, os)) {
                    preferred = pathList[i];
                    break;
                }
            }

            // Many PC games only document Windows (or Steam) paths; useful on Linux via Proton/Wine.
            return preferred || windowsFallback || steamFallback || anyFallback || { "System": os, "Location": null };
        }
                            ...
                            $scope.restoreVersions.tree = $("#restoreTree").fancytree({
                                ...
                                renderColumns: function (event, data) {
                                    ...
                                    var node = data.node,
                                        ...
                                        template;
                                    if (node.folder) {
                                        template = '<div ng-include="\'vaporcito/folder/restoreVersionsMassActions.html\'"/>';
                                    } else {
                                        template = '<div ng-include="\'vaporcito/folder/restoreVersionsVersionSelector.html\'"/>';
                                    }
        ...
        $scope.docsURL = function (path) {
            var url = 'https://github.com/alexiscaspell/vaporcito';
            if (!path) {
                return url + '#readme';
            }
            // Product docs site is not hosted yet; send users to the repo.
            return url;
        };
        ...
        $scope.shareDeviceIdDialog = function (method) {
            ...
            var title = $translate.instant('Vaporcito device ID for "{%devicename%}"', { devicename: deviceName });
            var footer = $translate.instant("Learn more at {%url%}", { url: "https://github.com/alexiscaspell/vaporcito" });
            ...
            switch (method) {
                case "email":
                    ...
                    params.body = [
                        $translate.instant('To connect with the Vaporcito device named "{%devicename%}", add a new remote device on your end with this ID:', { devicename: deviceName }),
                        deviceID,
                        $translate.instant("Vaporcito is a continuous file synchronization program. It synchronizes files between two or more computers in real time, safely protected from prying eyes. Your data is your data alone and you deserve to choose where it is stored, whether it is shared with some third party, and how it's transmitted over the internet."),
                        footer
                    ].join('\r\n\r\n');
                    break;
                case "sms":
    ...
    .directive('shareTemplate', function () {
        return {
            templateUrl: 'vaporcito/core/editShareTemplate.html',
            scope: {
                selected: '=',
                encryptionPasswords: '=',
                id: '@',
